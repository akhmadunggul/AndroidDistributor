...
object LicenseStore {
    ...
    fun getLabel(ctx: Context): String       = prefs(ctx).getString(K_LABEL,  "") ?: ""
    fun isRegistered(ctx: Context): Boolean  = getLabel(ctx) == "Terdaftar"
    ...
    fun save(ctx: Context, key: String, status: String, expiryMs: Long, days: Int, label: String = "") {
        prefs(ctx).edit()
            .putString(K_KEY,       key)
    ...
    fun updateCheck(ctx: Context, status: String, expiryMs: Long, days: Int, label: String = "") {
        prefs(ctx).edit()
            .putString(K_STATUS,   status)
            .apply { if (label.isNotBlank()) putString(K_LABEL, label) }
            .putLong(K_EXPIRY_MS,  expiryMs)
            .putInt(K_DAYS,        days)
            .putLong(K_LAST_CHECK, System.currentTimeMillis())
            .apply()
